        ...
        public IEnumerable<object> Get()
        {
            ...
            var query = from a in db.Debates
                        ...
                        select new
                        {
                            ...
                            Estado = a.Estado,
                            Rate = (from c in db.Ratings where c.DebateId == a.Id && c.CommentId == 0 && c.AutorId == b.Id select c.Rate).Take(1).Sum(),
                            RatingCount = (from d in db.Ratings where d.DebateId == a.Id && d.CommentId == 0 select d).Count(),
                            Average = (from d in db.Ratings where d.DebateId == a.Id && d.CommentId == 0 select d.Rate).Sum() /
